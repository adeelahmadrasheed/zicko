 ...
 */
function furnihome_unregister_default_widgets() {
...
add_action('widgets_init', 'furnihome_unregister_default_widgets', 11);
 
/*
** Register Sidebar and Widgets
*/
function furnihome_widgets_init() {
	
	// Sidebars
	global $furnihome_widget_areas;
	$furnihome_widget_areas = furnihome_widget_setup_args();
	if ( count($furnihome_widget_areas) ){
		foreach( $furnihome_widget_areas as $sidebar ){
			$sidebar_params = apply_filters('furnihome_sidebar_params', $sidebar);
			register_sidebar($sidebar_params);
		}
	}

	// Widgets
	register_widget('Furnihome_Social_Widget');
	register_widget('Furnihome_Posts_Widget');
	register_widget('Furnihome_Top_Widget');
}
add_action('widgets_init', 'furnihome_widgets_init');
 ...
 * @since 2.8.0
*/
class Furnihome_Posts_Widget extends Furnihome_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'sw_posts', 'description' => esc_html__('SW Posts', 'furnihome'));
		parent::__construct('sw_posts', esc_html__('SW Posts', 'furnihome'), $widget_ops);
		$this->base = get_template_directory().'/lib';
	}
}

class Furnihome_Top_Widget extends Furnihome_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'sw_top', 'description' => esc_html__('SW top header widget', 'furnihome'));
		parent::__construct('sw_top', esc_html__('SW Top Widget', 'furnihome'), $widget_ops);
	}
}

class Furnihome_Social_Widget extends WP_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'sw_social', 'description' => esc_html__('SW Social Networks', 'furnihome'));
		parent::__construct('sw_social', esc_html__('SW Social', 'furnihome'), $widget_ops);
	...
	function form($instance){ 
		...
		?>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title: ', 'furnihome'); ?>
...
<?php 
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo esc_html__( 'Social', 'furnihome' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'furnihome')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'furnihome')?> </label>
...
<?php
		...
		// furnihome fields for add new social network
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'furnihome' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'furnihome'); ?>
	</label> <input class="widefat" id="<?php echo esc_attr( $id_i_icon ); ?>"
		name="<?php echo esc_attr( $name_i_icon ); ?>" type="text" value=""
		placeholder="<?php esc_attr_e( 'Enter Font Awesome icon or Title', 'furnihome' ) ?>" /> <span><?php esc_html_e( 'If using as
		icon, please choose class name in Font Awesome. This is required.', 'furnihome' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'furnihome')?> </label>
...
class Furnihome_Widgets{
	...
	protected $fields = array(
			'furnihome_before_widget' => 'Before Widget',
			'furnihome_after_widget'  => 'After Widget',
			'furnihome_before_title'  => 'Before Title',
			'furnihome_after_title'   => 'After Title',
			'furnihome_display_conditions' => 'Display Conditions',
	...
	public function widget_display_callback( $instance, $widget, $args ){		
		...
		if ( furnihome_check_wdisplay($widget_display) == false ) return false;
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		$widget_advanced = furnihome_options()->getCpanelValue('widget-advanced');
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'furnihome' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'furnihome' ) ?>
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => esc_html__('All', 'furnihome'),
				'if_selected' => esc_html__('Only on selected', 'furnihome'),
				'if_no_selected' => esc_html__('Except selected', 'furnihome')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'furnihome' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => esc_html__('Show only for login users' , 'furnihome'),
				'logout' => esc_html__('Show only for logout users' , 'furnihome')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'furnihome' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => esc_html__('Blog posts', 'furnihome'),
					'archive' => esc_html__('Archive page', 'furnihome'),
					'date' => esc_html__('Archive Date', 'furnihome'),
					'category' => esc_html__('Category archive', 'furnihome'),
					'tag' => esc_html__('Tag archive', 'furnihome'),
					'author' => esc_html__('Author archive', 'furnihome'),
					'search' => esc_html__('Search page', 'furnihome'),
					'single' => esc_html__('Single post', 'furnihome'),
					'page' => esc_html__('Page template', 'furnihome'),
					'404' => esc_html__('Page 404', 'furnihome')
				);
			$html = '';
				...
				if(!isset($menu) || $menu != NULL || $menu != "")
					{
						...
						if (count($items) > 0) {
							$menu_checkbox = new Furnihome_Menu_Checkbox( $menu->slug);
	...
	protected function getWidgetStyles(){
		...
		if ( is_null($this->styles) ){
			$tmp = array();
			if ( $_core_styles = $this->scandir(FURNI_DIR.'/widgets/_styles', 'php') )
	...
	protected function loadStyle( $style = '' ){
		...
		if ( !empty($style) ){
			...
			$_core_style = FURNI_DIR.'/widgets/_styles/'.$style.'.php';
